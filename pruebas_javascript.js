...
function limitRandomIntInterval(limit, min, max) {
    ...
    if (limit <= 0) {
        errores++;
        elementoNumGenerados.append("El valor del numero limite no puede ser 0 o inferior. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero minimo sea inferior a cero y devolvemos un error si ese es el caso
    if (min < 0) {
        errores++;
        elementoNumGenerados.append("El valor del numero minimo no puede ser inferior a 0. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero maximo sea cero o inferior y devolvemos un error si ese es el caso
    if (max <= 0) {
        errores++;
        elementoNumGenerados.append("El valor del numero maximo no puede ser 0 o inferior. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero maximo sea mayor que 10000 y devolvemos un error si ese es el caso
    if (max > 10000) {
        errores++;
        elementoNumGenerados.append("El valor del numero maximo no puede superar 10000. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero minimo sea mayor que 10000 y devolvemos un error si ese es el caso
    if (min > 10000) {
        errores++;
        elementoNumGenerados.append("El valor del numero minimo no puede superar 10000. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero limite sea mayor que 10000 y devolvemos un error si ese es el caso
    if (limit > 10000) {
        errores++;
        elementoNumGenerados.append("El valor del numero limite no puede superar 10000. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    // comprobamos que el valor del numero maximo sea menor que el minimo y devolvemos un error si ese es el caso
    if (max < min) {
        errores++;
        elementoNumGenerados.append("El valor del numero maximo no puede ser inferior al valor del numero minimo. ");
        elementoNumGenerados.append(document.createElement("br"));
    }
    ...
    if (errores == 0) {
        ...
        elementoNumGenerados.innerHTML = numGenerados;
        elementoNumGenerados.append(document.createElement("br"));
    }
}
